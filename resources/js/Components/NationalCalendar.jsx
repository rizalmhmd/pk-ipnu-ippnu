                ...
                <div className="p-4 md:p-8">
                    <div className="grid grid-cols-7 gap-px bg-slate-100 rounded-lg overflow-hidden border border-slate-100">
                        ...
                        {calendarDays.map((day, idx) => {
                            ...
                            return (
                                <div key={idx} className={`min-h-[100px] md:min-h-[140px] bg-white p-2 md:p-3 group transition-colors flex flex-col ${day ? 'hover:bg-slate-50 relative' : ''}`}>
                                    {day && (
                                        <>
                                            <div className="flex justify-between items-start mb-2">
                                                <span className={`text-sm font-bold ${isToday ? 'w-8 h-8 rounded-full bg-emerald-600 text-white flex items-center justify-center' : (hasHoliday || isSunday ? 'text-red-500' : 'text-slate-400')}`}>
                                                    {day}
                                                </span>
                                            </div>
                                            <div className="space-y-1 overflow-y-auto max-h-[80px] md:max-h-[100px] no-scrollbar">
                                                {events.map((event, eIdx) => (
                                                    <button
                                                        ...
                                                        className={`w-full text-left text-[8px] md:text-[9px] p-2 rounded-lg font-bold leading-tight line-clamp-2 transition-all hover:brightness-95 active:scale-95 ${event.type === 'holiday'
                                    ...
                                    <div className="flex gap-4 items-center">
                                        ...
                                        <div>
                                            <span className={`px-4 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${selectedEvent.type === 'holiday'
                                                ...
                                                {selectedEvent.type === 'holiday' ? (selectedEvent.cat === 'nasional' ? 'Libur Nasional' : 'Hari Besar') : 'Agenda IPNU'}
                                            </span>
